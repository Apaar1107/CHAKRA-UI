...
const Videos = () => {
  ...
  return (
      <Stack direction={["column","row"]} h={"100%"}>
        ...
        <VStack w={["full","xl"]} alignItems={"stretch"} overflowY={"auto"} spacing={"8"} p={"8 "}>

            {videosArr.map((item,index)=>(<Button onClick={()=>setVideoSrc(item)} variant={"ghost"} colorScheme='purple' key={item.key}>
         Lecture {index+1}
          </Button> ))}
